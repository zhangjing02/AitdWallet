...
import com.quincysx.crypto.utils.Base58ForXrp;
...
public class B58 {
    ...
    public byte[] encodeToBytesChecked(byte[] input, byte[] version) {
        ...
        try {
            Log.e("test", "111=>>=" + new String(encodeToBytes(output), "US-ASCII"));
            Log.e("test", "222=>>=" + Base58ForXrp.encode(output));
            Log.e("test", "333=>>=" + Base58ForXrp.encodeToBytes(output));
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
