...
public class UsdtTransactionNativeActivity extends BaseActivity {

    private static final String TAG = "uuuuuuuuuuuuuuuuu";
    ...
    private void initWallet() {
        ...
        Log.i(TAG, master.getAddress() + "----initWallet: 这个私钥和地址----" + master.getPrivateKey());
        getBtcUtxo();
    }

    private void getToolBar() {
        toolbarTitle.setText(R.string.tittle_usdt_transaction);
    ...
    private void getBtcUtxo() {
        Observable.create((ObservableOnSubscribe<Response>) emitter -> {
            ...
            Gson gson = new Gson();
            Response response = HttpClientUtil.getInstance().postUsdtJson(gson.toJson(listunspentParams));
            if (response != null) {
    ...
    @SuppressLint("CheckResult")
    private void createTxToBroadcastApi(String omni_tx_hex) {
        Observable.create((ObservableOnSubscribe<List<GetUtxoBean.ResultBean>>) emitter -> {
            ...
            if (!Constant.HTTP_ERROR.equals(response)) {
                Response tx_response = HttpClientUtil.getInstance().postUsdtJson(jointOmniToTxParams(response, omni_tx_hex));
                if (tx_response != null) {
            ...
            //把拼接交易补上地址。
            String join_address_tx_hex = null;
            if (!Constant.HTTP_ERROR.equals(response)) {
                Response tx_response = HttpClientUtil.getInstance().postUsdtJson(jointAddressToTxParams(response));
                if (tx_response != null) {
                    if (tx_response.isSuccessful()) {
                        ResponseBody body = tx_response.body();
                        if (body != null) {
                            GetSimpleRpcBean bean = new Gson().fromJson(body.string(), GetSimpleRpcBean.class);
                            join_address_tx_hex = bean.getResult();
                        }
                    }
                }
            } else {
                join_address_tx_hex = Constant.HTTP_ERROR;
            }
            return join_address_tx_hex;
        }).map(response -> {
            //把补上地址的交易hex，添加找零地址，和费用信息。
            String join_fee_tx_hex = null;
            if (!Constant.HTTP_ERROR.equals(response)) {
                Response tx_response = HttpClientUtil.getInstance().postUsdtJson(jointFeeToTxParams(response));
                if (tx_response != null) {
                    if (tx_response.isSuccessful()) {
                        ResponseBody body = tx_response.body();
                        if (body != null) {
                            GetSimpleRpcBean bean = new Gson().fromJson(body.string(), GetSimpleRpcBean.class);
                            join_fee_tx_hex = bean.getResult();
                        }
                    }
                }
            } else {
                join_fee_tx_hex = Constant.HTTP_ERROR;
            }
            return join_fee_tx_hex;
        }).map(response -> {
            //把交易的hex，做签名处理。
            String tx_sign_hex_str = null;
            if (!Constant.HTTP_ERROR.equals(response)) {
                ...
                makeBroadcastTxParams(HexUtils.toHex(sign));
...
//                }
    ...
    private String createTransaction(List<GetUtxoBean.ResultBean> utxo_for_pay_list) {
        ...
        change_value = total_listunspent_value - miner_fee_total;
        ...
        Log.i(TAG, "createTransaction: 我们看一下添加的负载费用找零是多少？" + change_value);
        ...
        try {
            ...
            tx_hex = Constant.HTTP_ERROR;
            e.printStackTrace();
        }
        Log.i(TAG, "createTransaction: 我们看得到的交易hex是？" + tx_hex);
        if (!tx_hex.equals(Constant.HTTP_ERROR)) {
...
//                break;
    ...
    private List<GetUtxoBean.ResultBean> getUtxoCountForPay() {
        ...
        total_listunspent_value = utxo_list.get(0).getAmount();
        ...
        if (total_listunspent_value >= miner_fee_single + usdt_consume_fee) {  //如果一个listunspent，就足以支付费用（300+546聪）就只拿一个去进行交易即可。
            miner_fee_total = miner_fee_single * 2 + usdt_consume_fee;  //因为一笔交易，需要一个输入，一个输出，所以是2.
            utxo_for_pay.add(utxo_list.get(0));
        } else {
            for (int i = 0; i < utxo_list.size(); i++) {
                miner_fee_total = miner_fee_single * (i + 2) + usdt_consume_fee;
                if (miner_fee_total < total_listunspent_value) {//如果支付金额+交易费小于拼装的utxo，就可以进行支付了，此时可以跳出循环。
                    break;
                } else {
                    total_listunspent_value = total_listunspent_value + utxo_list.get(i).getAmount();
    ...
    private String makeBroadcastTxParams(String transaction_hex) {
        ...
        Log.i(TAG, "makeOutputApiParams: 022222我们拼接的请求体是？" + new Gson().toJson(listunspentParams));
    ...
    private String makeOmniTxParams() {
        ...
        Log.i(TAG, "makeOutputApiParams: 033333333我们拼接的请求体是？" + new Gson().toJson(listunspentParams));
        return new Gson().toJson(listunspentParams);
    }
    ...
    private String jointOmniToTxParams(String load_hex, String omni_tx_hex) {
        ...
        Log.i(TAG, "makeOutputApiParams: 044444444我们拼接的请求体是？" + new Gson().toJson(listunspentParams));
        return new Gson().toJson(listunspentParams);
    }
    ...
    private String jointAddressToTxParams(String join_tx_hex) {
        ...
        Log.i(TAG, "makeOutputApiParams: 055555555我们拼接的请求体是？" + new Gson().toJson(listunspentParams));
        return new Gson().toJson(listunspentParams);
    }


    private String jointFeeToTxParams(String join_address_tx_hex) {
        Map<String, Object> listunspentParams = new HashMap<>();
        listunspentParams.put("jsonrpc", "1.0");
        listunspentParams.put("id", "testnet3");
        listunspentParams.put("method", "omni_createrawtx_change");
        List<Object> params = new ArrayList<>();

        params.add(join_address_tx_hex);//添加了地址的交易hex


        List<Map<String, Object>> pay_params = new ArrayList<>();
        for (int i = 0; i < utxo_for_pay.size(); i++) {
            Map<String, Object> pay_param = new HashMap<>();
            pay_param.put("txid", utxo_for_pay.get(i).getTxid());
            pay_param.put("vout", utxo_for_pay.get(i).getVout());
            pay_param.put("scriptPubKey", utxo_for_pay.get(i).getScriptPubKey());
            //  pay_param.put("value", utxo_for_pay.get(i).getAmount());
            pay_param.put("value", 0);
            pay_params.add(pay_param);
        }

        // total_params.add(pay_params);

        params.add(pay_params);    //添加需要用于支付费用的listunspent
        params.add(master.getAddress());//找零的地址 。
        params.add(DataReshape.doubleBig(miner_fee_total, 8));//费用 。
        listunspentParams.put("params", params);

        Log.i(TAG, "makeOutputApiParams: 0666666我们拼接的请求体是？" + new Gson().toJson(listunspentParams));
        return new Gson().toJson(listunspentParams);
    }
    ...
    private boolean judgeSelectInput() {
        if (TextUtils.isEmpty(etPaymentAddress.getText().toString())) {
            ToastUtil.showToast(this, getString(R.string.notice_input_receive_address));
            return false;
        } else if (TextUtils.isEmpty(etPaymentAmount.getText().toString())) {
            ToastUtil.showToast(this, getString(R.string.notice_input_tx_amount));
            return false;
        }
        return true;
    }
