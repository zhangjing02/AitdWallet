...
public class ImportCoinListActivity extends BaseActivity {
    ...
    private String coin_name,coin_id;
    ...
    protected void initView() {
       ...
       // mMessageBeans.addAll(specCoinInfo);
        mMessageBeans.add(mainCoinFrCoinId);
...
//        ViewPager mViewPager = header.findViewById(R.id.viewpager);
      ...
      //  List<CoinInfo> specCoinInfo1 = CoinInfoManager.getCoinFrIdInfo(Constant.TRANSACTION_COIN_NAME_BTC);
       ...
       // CoinInfo coinFrAddress = specCoinInfo1.get(0);
        WalletInfo wallet_btc = WalletInfoManager.getWalletFrId(mainCoinFrCoinId.getWallet_id());
        CurrencyCardBean mCurrencyCardBean;
        if (wallet_btc != null) {
            mCurrencyCardBean = new CurrencyCardBean();
            mCurrencyCardBean.setCoin_name(mainCoinFrCoinId.getCoin_name());
            mCurrencyCardBean.setCoin_alias_name(mainCoinFrCoinId.getAlias_name());
            mCurrencyCardBean.setCoin_address(mainCoinFrCoinId.getCoin_address());
            mCurrencyCardBean.setCoin_icon_id(mainCoinFrCoinId.getCoin_id());
            if (userInformation.getFiatUnit().equals(Constants.FIAT_USD)){
                mCurrencyCardBean.setCurrency_to_fiat_num("≈ $"+ DataReshape.doubleBig(mainCoinFrCoinId.getCoin_totalAmount() * wallet_btc.getCoin_USDPrice(),2));
            }else {
                mCurrencyCardBean.setCurrency_to_fiat_num("≈ ￥"+DataReshape.doubleBig(mainCoinFrCoinId.getCoin_totalAmount() * wallet_btc.getCoin_CNYPrice(),2));
            }
            switch (mainCoinFrCoinId.getCoin_name()){
